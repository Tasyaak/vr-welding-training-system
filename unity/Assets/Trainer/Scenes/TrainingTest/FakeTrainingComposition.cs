...
namespace WeldingTrainer.TrainingTest
{
    ...
    public sealed class FakeTrainingComposition : MonoBehaviour
    {
        ...
        void Awake()
        {
            coordinator = new TrainingCoordinator(ports, ports, ports, ports, ports, ports, ports, ports);
            coordinator.StartSession();
            Debug.LogWarning("FAKE TRAINING COMPOSITION: synthetic valid ports; never use in production Bootstrap.", this);
        }
        ...
        { int id; public double Seconds => Time.realtimeSinceStartupAsDouble; public string NewId() => (++id).ToString(); public TrainingInput Capture(double now) => new(now, 1, 1, true, true, true, false, true, false, false); RegistrationInput IRegistrationPort.Capture(double now) => new(true, true, 1, 1, "synthetic-fixture"); public void Teardown() { } public SafetyInput Evaluate(TrainingInput i, RegistrationInput r, AttemptConfiguration a) => new(true, false, true, true, false, new ContactEvidence(ContactState.Contact, ContactReason.None, "synthetic", default, new Vec3(0, 0, 1), 0, 0, true, true), RiskState.Low, BlockReason.None); public bool Available => true; public void Append(ProcessEvent e) { } public void Begin(AttemptConfiguration a) { } public bool Finish(string id, bool interrupted, out string error) { error = null; return true; } public void Publish(ProcessSnapshot s) { } public void StopImmediately() { } }
    }
}
