...
namespace WeldingTrainer.Domain
{
    ...
    public sealed class SeamCoverageTracker
    {
        ...
        public CoverageSnapshot Update(
            PathMetrics metrics,
            ActivationDecision activation,
            bool acceptable,
            long sampleGeneration,
            long requiredGeneration)
        {
            if (sampleGeneration != requiredGeneration)
            {
                Close(CoverageCloseReason.GenerationMismatch);
                return Snapshot();
            }
            ...
            if (!activation.Active)
            {
                Close(
                    activation.Requested
                        ? CoverageCloseReason.Inhibited
                        : CoverageCloseReason.Released);
            ...
            if (metrics == null)
            {
                Close(CoverageCloseReason.InvalidEvidence);
                return Snapshot();
            }

            MotionFlags discontinuityFlags =
                MotionFlags.Discontinuity |
                MotionFlags.SkippedRegion |
                MotionFlags.TrackingGap |
                MotionFlags.AmbiguousProjection |
                MotionFlags.NonMonotonicTime;

            if ((metrics.Flags & discontinuityFlags) != 0)
            {
                Close(CoverageCloseReason.Discontinuity);
                return Snapshot();
            }

            if (!metrics.Valid || !metrics.Projection.Valid)
            {
                Close(CoverageCloseReason.InvalidEvidence);
                return Snapshot();
            }

            double arc = metrics.Projection.ArcLengthMetres;
            ...
            if (!anchored)
            {
                ...
                priorArc = arc;
                segmentId++;
                closeReason = CoverageCloseReason.None;

                return Snapshot();
            }

            double distance = Math.Abs(arc - priorArc);
            ...
            if (distance > 1e-9)
            {
                CoverageQuality quality =
                    acceptable &&
                    !metrics.Flags.HasFlag(MotionFlags.Reverse)
                        ? CoverageQuality.Acceptable
                        : CoverageQuality.Poor;

                ApplyInterval(
                    Math.Min(priorArc, arc),
                    Math.Max(priorArc, arc),
                    quality,
                    segmentId);
