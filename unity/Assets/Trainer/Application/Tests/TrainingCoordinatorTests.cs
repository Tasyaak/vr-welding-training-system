...
namespace WeldingTrainer.Application.Tests
{
    public sealed class TrainingCoordinatorTests
    {
        ...
        [Test] public void EmergencyStopWinsSameTimestamp(){Ready();Prepare();coordinator.Submit(TrainingCommandType.Arm,clock.Seconds);coordinator.Submit(TrainingCommandType.EmergencyStop,clock.Seconds);coordinator.Tick();Assert.That(coordinator.Current.Activation,Is.EqualTo(ActivationState.ResetRequired));Assert.That(haptics.Count,Is.GreaterThan(0));Assert.That(haptics.Cancelled,Is.EqualTo(1));}
        [TestCase(ProcessMode.Fusion)] [TestCase(ProcessMode.Wobble)] [TestCase(ProcessMode.Pulsed)] [TestCase(ProcessMode.PreWeldCleaning)] [TestCase(ProcessMode.PostWeldCleaning)]
        public void EmergencyStopCancelsEveryMode(ProcessMode mode){Ready();coordinator.PrepareAttempt(Config(mode));coordinator.Submit(mode<=ProcessMode.Pulsed?TrainingCommandType.SelectWeldingNozzle:TrainingCommandType.SelectCleaningNozzle,clock.Seconds);coordinator.Submit(TrainingCommandType.ConnectClamp,clock.Seconds);coordinator.Tick();input.Value=Valid(false,true);coordinator.Tick();coordinator.Submit(TrainingCommandType.Arm,clock.Seconds);coordinator.Tick();coordinator.Submit(TrainingCommandType.EmergencyStop,clock.Seconds);coordinator.Tick();Assert.That(coordinator.Current.Activation,Is.EqualTo(ActivationState.ResetRequired));Assert.That(haptics.Cancelled,Is.EqualTo(1));}
        ...
        sealed class Safety:ISafetyPort{public SafetyInput Value=new(true,true,false,BlockReason.None);public SafetyInput Evaluate(TrainingInput i,RegistrationInput r,AttemptConfiguration a)=>Value;} sealed class Sink:ISnapshotSink{public ProcessSnapshot Last;public void Publish(ProcessSnapshot s)=>Last=s;} sealed class Haptics:IHapticStop,IProcessStopSink{public int Count,Cancelled;public void StopImmediately()=>Count++;public void StopAll(long epoch,BlockReason reason)=>Cancelled++;}
