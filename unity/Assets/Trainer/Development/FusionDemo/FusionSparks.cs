...
namespace WeldingTrainer.FusionDemo
{
    public sealed class FusionSparks : MonoBehaviour
    {
        ...
        [Range(20, 500)] public float sparksPerSecond = 140;
        ...
        private ParticleSystem CreateParticles(string label, bool stretched)
        {
            ...
            gradient.SetKeys(new[] {
                new GradientColorKey(new Color(1, 0.92f, 0.65f), 0),
                new GradientColorKey(new Color(1, 0.38f, 0.04f), 0.25f),
                new GradientColorKey(new Color(0.55f, 0.035f, 0.004f), 1)
            }, new[] { new GradientAlphaKey(1, 0), new GradientAlphaKey(0.9f, 0.45f), new GradientAlphaKey(0, 1) });
        ...
        public void SetWelding(bool active, Vector3 worldPoint, Vector3 worldNormal)
        {
            ...
            if (active != emitting)
            {
                if (active) { streaks.Play(); embers.Play(); }
                else { streaks.Stop(true, ParticleSystemStopBehavior.StopEmitting); embers.Stop(true, ParticleSystemStopBehavior.StopEmitting); }
                emitting = active;
            }
        ...
        {
            emitting = false;
            if (streaks != null) streaks.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
